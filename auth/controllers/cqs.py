...
async def receive_command(request: Command, service: Annotated[Service, Depends(service)]):
    try:
...
@router.post('/queries/')
async def receive_query(request: Query, service: Annotated[Service, Depends(service)]):
    try:
